...
class EbookViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        print("okok")
        Alamofire.request("https://itunes.apple.com/search?term=jack+johnson&entity=ebook").responseJSON { (response) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "ebookCell", for: indexPath) as! EbookTableViewCell
        if listSong.count != 0{
